...
public class Audio {
    ...
    @FXML
    void GetAuditory(ActionEvent actionEvent) throws IOException {
    ...
    void getAllLessons() throws IOException {
        if(textField.getText().isEmpty())
        {
            ...
            alert.setContentText("Введите номер аудитории!");
            ...
            if (teacherSchedule.isEmpty()) {
                ...
                alert.setContentText("Введенная аудитория не существуют!");
                alert.showAndWait().ifPresent(rs -> {
                    if (rs == ButtonType.OK) {
                        System.out.println("Pressed OK.");
                    }
    ...
    public  void getLesson(XSSFSheet sheet, int row, int cell, int lesson_number, int group_index,boolean flag, int day){
        ...
        try {
            ...
            if(getCellString(row_t.getCell(cell + 3)).equals(textField.getText()))
            {
